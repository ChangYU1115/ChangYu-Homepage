(function (c) {
  function ga(z, s) {
    function m() {
      ...
      a.settings.tooltip &&
        c('<div class="tooltip"><p></p></div>').css("opacity", 0).appendTo(i);
    }
    function ha(b) {
      var d = c('<img class="carousel-item"/>').appendTo(i);
      Z.push(d);
      d.css({ width: a.settings.itemWidth, h: a.settings.itemHeight })
        .data({ w: a.settings.itemWidth, h: a.settings.itemHeight, index: b })
        .addClass("out")
        .bind({
          mouseover: function () {
            c(this).hasClass("out") &&
              c(this).removeClass("out").addClass("over");
          ...
          click: function () {
            ...
            if (j[b].link)
              window.open(j[b].link, j[b].linkTarget || a.settings.linkTarget);
            var e = { type: "itemClick", index: b, data: j[b] };
    ...
    function o(b) {
      i.find(".carousel-item").each(function (d) {
        var e = c(this),
          ...
          f = e.data("h"),
          D =
            Math.sin(-(p * d) + v * 0.5 + b * w) * a.settings.verticalRadius +
            P -
            f * 0.5,
          r =
            ((D - ($ - f * 0.5)) / (aa - $)) * (1 - a.settings.scaleRatio) +
            a.settings.scaleRatio;
        e.css({
          width: h * r,
          height: f * r,
          left:
            Math.cos(-(p * d) + v * 0.5 + b * w) * a.settings.horizontalRadius +
            Q -
            h * 0.5 +
            (h * (1 - r)) / 2,
          top: D + (f * (1 - r)) / 2,
    ...
    function X() {
      ...
      function d() {
        ...
        J = setInterval(function () {
          var f =
            (360 * ((h * (T - da)) / (100 * a.settings.mouseDragSpeed)) +
              e -
              g) *
    ...
    function ia() {
      ...
      function d() {
        if (n < 0) n = 0;
        else if (n > parseInt(h.css("width")) - parseInt(f.css("width")))
